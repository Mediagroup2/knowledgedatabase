...
error_reporting(E_ALL ^ E_NOTICE);
 ...
 $toegevoegd_op = mysql_real_escape_string($_POST['toegevoegd_op']);
  $queryEdit = mysql_query("UPDATE psd SET titel = '$titel', tags = '$tags', categorie = '$categorie', url = '$url', beschrijving = '$beschrijving', toegevoegd_op = '$toegevoegd_op', WHERE id= '$id'");
  if ($queryEdit) {
           header('location: toevoegen_psd.php');

  }
if (!empty($titel) OR ($url)) {
 $query = mysqli_query($con, "INSERT INTO psd (titel, tags, categorie, url, beschrijving, toegevoegd_op) VALUES ('$_POST[titel]','$_POST[tags]','$_POST[categorie]','$_POST[url]','$_POST[beschrijving]','$_POST[toegevoegd_op]')");
  mysql_close();
